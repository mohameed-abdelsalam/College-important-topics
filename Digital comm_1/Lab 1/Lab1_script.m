...
bit_seq = GenerateBits(N_bits);           %generate seq of bits
p_vect_4 = 1:1:20;                        % Use this vector to extract different values of repetitions in your code
BER_case_4_vec = zeros(size(p_vect_4));   % Use this vector to store the resultant BER

% generate seq--> select repetion range -->create vector of same size to
% copare o/p with it
% --> get sample sequence according to Fs --> pass sample seq throw chaanel BSC 
%--> then throw RX --> claculate BER 

for fs_ind = 1:length(p_vect_4)

    sample_seq = GenerateSamples(bit_seq,p_vect_4(fs_ind));
    rec_sample_seq = BSC(sample_seq,p_vect_4(fs_ind),p);
    rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_2',p_vect_4(fs_ind));
    BER_case_4_vec(fs_ind) = ComputeBER(bit_seq,rec_bit_seq);
end

figure(2)
plot(p_vect_4,BER_case_4_vec,'o-r','linewidth',2);
